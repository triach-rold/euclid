...
int main() {
    for (int n = -1; n <= 30; ++n) {
        std::cout <<  PURPLE_TEXT "n = " << n << RESET_TEXT << std::endl;
        ...
        std::cout <<  CYAN_TEXT << "Error Percentage: " <<  ((factorial(n) - sterling_factorial(n)) / static_cast<double>(factorial(n))) * 100 << "%" << RESET_TEXT << std::endl;
        std::cout << std::endl;
    }
    return 0;
}
